...
for (const user of db.users) {
  if (!user.userId) issues.push(`Users: missing userId for ${user.displayName || user.lineProfileId || user.lineUserId}`);
...
for (const enrollment of db.enrollments) {
  if (!enrollment.userId) issues.push(`Enrollments: ${enrollment.enrollmentId} is missing userId`);
